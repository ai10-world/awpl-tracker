...
import { Users, Trash2, Edit2, Check, X, Loader2, Crown, Star, Search } from "lucide-react";
...
export function MemberList({
  members, teamId, currentUserId, canManage, isAdmin,
}: {
  members: any[]; teamId: string; currentUserId: string; canManage: boolean; isAdmin: boolean;
}) {
  ...
  const filtered = members.filter((m) => {
    ...
    return (
      ...
      m.profile.awpl_id?.toLowerCase().includes(q) ||
  ...
  return (
    <div>
        ...
        {members.length > 0 && (
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <input
              ...
              placeholder="Search by name or AWPL ID…"
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-8 pr-3 py-2 text-xs text-white placeholder:text-white/25 focus:outline-none focus:border-brand-500/40 transition-all"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
      ...
      {search && filtered.length === 0 && (
        <div className="text-center py-6 border border-white/5 rounded-xl text-white/25 text-xs">
      ...
      <div className="space-y-2">
        {filtered.map((m: any) => {
          ...
          return (
            <div key={m.id}
              className="p-3.5 rounded-xl bg-white/3 border border-white/8 hover:border-white/12 transition-all">
              {isEditing ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => saveEdit(m)} disabled={isLoading}
                      ...
                      <button onClick={() => setEditingId(null)}
                        className="text-xs text-white/40 border border-white/10 px-2.5 py-1.5 rounded-lg">
                        <X size={11} />
                      </button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <input type="number" value={editRank} onChange={(e) => setEditRank(parseInt(e.target.value))}
                        min="0" max="100"
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none" />
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-2 text-xs text-white/40 cursor-pointer">
                      <input type="checkbox" checked={editCanAssign} onChange={(e) => setEditCanAssign(e.target.checked)} className="accent-orange-500" />
                      Can assign tasks
                    </label>
                    <label className="flex items-center gap-2 text-xs text-white/40 cursor-pointer">
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-8 h-8 rounded-full bg-white/8 flex items-center justify-center font-display font-semibold text-xs text-white/70 flex-shrink-0">
                      {m.profile.full_name?.[0]?.toUpperCase()}
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="font-medium text-sm truncate">{m.profile.full_name}</span>
                        {isCurrentUser && <span className="text-xs text-white/25 flex-shrink-0">(you)</span>}
                      </div>
                      <div className="text-xs text-white/30 font-mono">{m.profile.awpl_id}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border ${roleColors[m.role]}`}>
                      ...
                      <span className="hidden sm:inline">{roleLabels[m.role]}</span>
                    </span>
                    {canManage && !isCurrentUser && (
                      <>
                        ...
                        <button onClick={() => handleRemove(m)} disabled={isLoading}
