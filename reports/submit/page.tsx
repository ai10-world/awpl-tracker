...
import { ArrowLeft, Loader2, CheckCircle, AlertCircle, Globe, ClipboardList, Edit3 } from "lucide-react";
...
export default function SubmitReportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    fd.set("team_id", selectedTeam); fd.set("mood", mood);
    ...
    if (result?.success) {
      ...
      fetch(`/api/reports/today?team_id=${selectedTeam}`)
        .then((r) => r.json()).then((data) => { if (data.report) setExistingReport(data.report); });
    }
  };
    ...
    <div className="min-h-screen pb-24 lg:pb-8" style={{ background: "var(--bg)" }}>
      ...
      <div className="lg:ml-60">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 lg:py-8">
          ...
          {/* Desktop back + language toggle */}
          <div className="hidden lg:flex items-center justify-between mb-6">
            <Link href="/reports" className="flex items-center gap-2 text-sm transition-colors"
          ...
          <div className="relative overflow-hidden rounded-3xl p-6 mb-5 text-center"
            ...
            <div className="relative">
              ...
              <h1 className="font-display font-bold text-2xl sm:text-3xl mb-0.5"
                style={{ color: "var(--text-1)" }}>
                *{t.greeting}*
              </h1>
              <p className="font-medium text-sm sm:text-base"
                style={{ color: "rgba(255,163,74,0.8)" }}>
                *{t.subtitle}*
              </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {moods.map((m) => (
                  <button key={m.value} type="button" onClick={() => setMood(m.value)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-medium transition-all ${
                      mood === m.value ? m.color : "text-white/30 hover:border-white/20"
                    }`}
                    style={mood !== m.value ? { borderColor: "var(--border-1)", background: "transparent" } : {}}>
            ...
            <div className="card overflow-hidden divide-y" style={{ borderColor: "var(--border-1)" }}>
              {[
                ...
                <div key={field.name} className="p-4" style={{ borderColor: "var(--border-1)" }}>
              ...
              {/* SP */}
              <div className="p-4" style={{ borderColor: "var(--border-1)" }}>
                <label className="flex items-center gap-2 text-sm font-semibold mb-2"
                  style={{ color: "#fbbf24" }}>
                  ⭐ {t.sp} —
                </label>
