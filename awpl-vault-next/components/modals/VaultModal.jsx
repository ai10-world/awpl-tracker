...
// ── Create Vault Modal ────────────────────────────────────────────────────────
export function VaultModal({ onSave, onClose }) {
  ...
  function save() {
    ...
    onSave({ name: name.trim(), teamId: teamId.trim().toUpperCase() })
  }
    ...
    <Overlay onClose={onClose}>
      <div className="w-full max-w-sm">
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <Field label="Team ID *">
            <input value={teamId} onChange={e => { setTeamId(e.target.value); setError('') }}
                   placeholder="e.g. TEAM001 or your Dist. ID"
                   className="form-input font-mono tracking-wider" />
            <p className="text-xs" style={{ color: 'var(--text3)' }}>
              Use your distributor ID or a unique team identifier
            </p>
          </Field>
    ...
    <Overlay onClose={onClose}>
      <div className="w-full max-w-sm text-center">
        <div className="p-6 flex flex-col items-center gap-4">
          ...
          <div>
            <h3 className="font-syne font-bold text-lg mb-1">Delete this entry?</h3>
            <p className="text-sm" style={{ color: 'var(--text2)' }}>
              <span className="font-semibold" style={{ color: 'var(--text)' }}>{entryName}</span>
              {' '}will be permanently removed along with all photos.
            </p>
          </div>
    ...
    <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="modal-panel w-full flex justify-center">
        <div className="w-full rounded-2xl border overflow-hidden"
             style={{ background: 'var(--surface)', borderColor: 'var(--border2)', maxWidth: '440px' }}>
          {children}
        </div>
      </div>
...
function ModalHeader({ icon, title, onClose }) {
  return (
    <div className="flex items-center gap-3 p-5 border-b" style={{ borderColor: 'var(--border)' }}>
      ...
      <span className="font-syne font-bold text-base flex-1">{title}</span>
